...
static enum Iso14229ResponseCodeEnum onRequest(void *userCtx, const uint8_t dataFormatIdentifier,
...
static enum Iso14229ResponseCodeEnum onTransfer(void *userCtx, uint8_t *data, uint32_t len) {
    UDSBootloaderInstance *self = (UDSBootloaderInstance *)userCtx;
    bufferedWriterWrite(&self->bufferedWriter, data, len);
    return kPositiveResponse;
}

static enum Iso14229ResponseCodeEnum onExit(void *userCtx) {
    UDSBootloaderInstance *self = (UDSBootloaderInstance *)userCtx;
    bufferedWriterFinalize(&self->bufferedWriter);
    return kPositiveResponse;
}

enum Iso14229ResponseCodeEnum startEraseAppProgramFlashRoutine(void *userCtx,
...
int udsBootloaderInit(void *vptr_self, const void *vptr_cfg, Iso14229Instance *iso14229) {
    ...
    iso14229UserEnableService(iso14229, kSID_DIAGNOSTIC_SESSION_CONTROL);
    iso14229UserEnableService(iso14229, kSID_COMMUNICATION_CONTROL);
    iso14229UserEnableService(iso14229, kSID_ROUTINE_CONTROL);
    iso14229UserEnableService(iso14229, kSID_ECU_RESET);
    iso14229UserEnableService(iso14229, kSID_READ_DATA_BY_IDENTIFIER);
    iso14229UserEnableService(iso14229, kSID_WRITE_DATA_BY_IDENTIFIER);
    iso14229UserEnableService(iso14229, kSID_REQUEST_DOWNLOAD);
    iso14229UserEnableService(iso14229, kSID_TRANSFER_DATA);
    iso14229UserEnableService(iso14229, kSID_REQUEST_TRANSFER_EXIT);
    iso14229UserEnableService(iso14229, kSID_TESTER_PRESENT);

    self->startup_20ms_timer = iso14229UserGetms() + 20,
    ...
    if (0 != iso14229UserRegisterRoutine(iso14229, &self->eraseAppProgramFlashRoutine)) {
        return -1;
    }

    if (0 != iso14229UserRegisterDownloadHandler(iso14229, &self->dlHandler, &self->dlHandlerCfg)) {
        return -1;
    }
 ...
 * @param self
 */
static inline int udsBootloaderStateMachine(UDSBootloaderInstance *self,
    ...
    case kBootloaderSMStateWaitForTesterPresent:
        if (kDiagModeExtendedDiagnostic == iso14229->diag_mode) {
            self->sm_state = kBootloaderSMStateDiagnosticSession;
        } else if (Iso14229TimeAfter(iso14229UserGetms(), self->startup_20ms_timer)) {
    ...
    case kBootloaderSMStateDiagnosticSession:
        if (Iso14229TimeAfter(iso14229UserGetms(), iso14229->s3_session_timeout_timer)) {
...
int udsBootloaderPoll(void *vptr_self, Iso14229Instance *iso14229) {
    UDSBootloaderInstance *self = (UDSBootloaderInstance *)vptr_self;
